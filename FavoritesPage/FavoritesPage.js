...
//data.filter(item => Number(input) >= Number(item[option].replace(/\D/g, "")));
...
const FavoritesPage = () => {
    ...
    useEffect(() => {
        ...
        const newData = myData.filter(item => Number(input) >= Number(item[option].replace(/\D/g, "")))
        setFiltered(newData)
